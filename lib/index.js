	...
	if (!(command in this)) {
		...
		var fn = function executor () {
			...
			var callback = getCallback(arguments[arguments.length - 1]),
				args = marshal(Array.prototype.slice.call(arguments, 0, callback ? -1 : arguments.length));
...
Multi.prototype.exec = function (callback) {
	var self = this;
	async.map(this._commands, function (args, cb) {
		var command = args[0],
			arglist = args.slice(1),
			last = arglist.length && arglist[arglist.length - 1];
		if (last && typeof(last) === 'function') {
			// we have a callback so we need to first delegate to it and then
			// call our map callback to collect the responses
			arglist[arglist.length - 1] = function (err, reply) {
				last(err, reply);
				if (err) {
					cb(err);
				} else {
		...
		self.cache[command].apply(self.cache, arglist);
	}, callback);
};
